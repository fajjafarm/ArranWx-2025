            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="text-center">
                        ...
                        @if (isset($weatherData['current']))
                            <p class="mb-1 text-muted">
                                <span class="me-2">Wind: {{ $weatherData['current']['wind_speed'] ?? 'N/A' }} m/s ({{ $controller->degreesToCardinal($weatherData['current']['wind_from_direction'] ?? null) }})</span>
                                <span>Humidity: {{ $weatherData['current']['relative_humidity'] ?? 'N/A' }}%</span>
                            </p>
                            @if ($weatherData['type'] === 'Marine' && isset($weatherData['marine']))
                                <p class="mb-1 text-muted">
                                    <span class="me-2">Wave Height: {{ $weatherData['marine']['wave_height'] ?? 'N/A' }} m</span>
                                    <span>Sea Temp: {{ $weatherData['marine']['sea_surface_temperature'] ?? 'N/A' }}°C</span>
                                </p>
                            @endif
                        @endif
                    </div>
                    ...
                    @if (!empty($weatherData['hourly']))
                            ...
                            <table class="forecast-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Rain (mm)</th>
                                        <th>Pressure (hPa)</th>
                                        @if ($weatherData['type'] === 'Marine' && isset($weatherData['marine_hourly']))
                                            <th>Wave Height (m)</th>
                                            <th>Wave Dir</th>
                                        @endif
                                    </tr>
                                ...
                                <tbody>
                                    @foreach (collect($hours)->filter(function ($hour, $index) { return $index % 2 === 0; }) as $hour)
                                        @php
                                            ...
                                            // Find matching marine hourly data
                                            $marineHour = null;
                                            if ($weatherData['type'] === 'Marine' && isset($weatherData['marine_hourly'])) {
                                                $hourTime = \Carbon\Carbon::parse($date . ' ' . $hour['time'])->format('Y-m-d\TH:i');
                                                $marineHour = collect($weatherData['marine_hourly'])->firstWhere('time', $hourTime);
                                            }
                                        @endphp
                                        <tr>
                                            <td>{{ $hour['time'] }}</td>
                                            ...
                                            @if ($weatherData['type'] === 'Marine' && $marineHour)
                                                <td>{{ $marineHour['wave_height'] ?? 'N/A' }}</td>
                                                <td>
                                                    @if (isset($marineHour['wave_direction']))
                                                        <div class="wind-direction" title="{{ $controller->degreesToCardinal($marineHour['wave_direction']) }} ({{ $marineHour['wave_direction'] }}°)">
                                                            <div class="wind-arrow" style="transform: translate(-50%, -50%) rotate({{ $marineHour['wave_direction'] - 180 }}deg);">
                                                                <div class="wind-arrow-tail" style="height: {{ $marineHour['wave_height'] * 10 }}px; @media (max-width: 768px) { height: {{ $marineHour['wave_height'] * 6 }}px; }"></div>
                                                            </div>
                                                        </div>
                                                    @else
                                                        N/A
                        ...
                        @endforeach
                        <p class="text-muted fs-12 mt-2">* Gusts estimated based on wind speed, weather conditions, and {{ $weatherData['type'] === 'Hill' ? 'altitude' : ($weatherData['type'] === 'Marine' ? 'marine factors' : 'rural factors') }}.</p>
